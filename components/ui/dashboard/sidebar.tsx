    ...
    <>
                  ...
                  <DropdownMenu.Group>
                    ...
                    {hiddenItems.map((item) => (
                      <DropdownMenu.Item key={item.label} asChild color="green">
                        <Link href={item.href} className="flex size-8 items-center justify-center sm:size-14">
      ...
      <aside className="m-5 hidden w-64 flex-col gap-5 rounded-xl bg-white px-5 py-10 md:flex lg:w-80">
        ...
        <nav className="mt-5 flex flex-col gap-8">
          {Object.entries(categories).map(([category, { index, items: catItems }]) => (
            <div key={category} className="space-y-2">
              ...
              <ul className="flex flex-col gap-3">
                {catItems.map((item, itemIdx) => {
                  const active = isActive(index + itemIdx)

                  return (
                    <Link
                      href={item.href}
                      key={item.label}
                      className={`flex w-full items-center bg-[${
                        active ? "#00CF68" : "#FAFAFA"
                    ...
                    >
                      ...
                      <Text
                        ...
                        className={`${
                          isActive(index + itemIdx) ? "text-white" : "text-[#333]"
                        } lg:text-medium text-base`}
                      >
                        {item.label}
                      </Text>
                    </Link>
